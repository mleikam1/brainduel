...
class SharedQuizScreen extends ConsumerStatefulWidget {
  const SharedQuizScreen({super.key, required this.quizId});

  final String quizId;

  @override
  ConsumerState<SharedQuizScreen> createState() => _SharedQuizScreenState();
}

class _SharedQuizScreenState extends ConsumerState<SharedQuizScreen>
  ...
  Widget build(BuildContext context) {
    ...
    if (!_started && userReady) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        ref.read(quizControllerProvider.notifier).loadSharedQuiz(widget.quizId);
        _started = true;
      });
    }
                                ...
                                children: [
                                  Wrap(
                                    ...
                                    children: [
                                      ...
                                      BDStatPill(label: 'Mode', value: 'Shared', icon: Icons.flash_on),
                                    ],
                                  ),
                                  const SizedBox(height: 16),
                                  Text(
